...
public class IcebergRenameOracle {
 public static void main(String[] args) throws Exception {
  ...
  for(var c:mapper.readTree(base.resolve("results.json").toFile()).get("results")){
   ...
   Schema schema=new SchemaUpdate(original.schema(),original.lastColumnId()).renameColumn(original.schema().findColumnName(c.get("fieldId").asInt()),c.get("newName").asText()).apply();
   ...
   for(String f:new String[]{"json","yaml"}){
    ...
    if(!expectedJson.equals(actualJson))throw new IllegalStateException("Native rename differs: "+id+" "+f+"\n"+expectedJson+"\n"+actualJson);
   }
   out.add(mapper.createObjectNode().put("id",id).put("formatsAgree",2).put("nativeOperation","SchemaUpdate.renameColumn + TableMetadata.updateSchema").put("excludedComparison","last-updated-ms: UMF candidate does not advance commit time"));
  }
  mapper.writerWithDefaultPrettyPrinter().writeValue(base.resolve("java-results.json").toFile(),out);System.out.println("Native rename candidates checked: "+out.size());
 }
}
