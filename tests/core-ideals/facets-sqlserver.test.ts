...
test('native counterexamples remain explicit and strict/report results differ',()=>{
 ...
 for(const table of ['disabled','custom','length_len','length_sentinel','length_zero','alias_value','nvarchar1']){expect(run(table,'value',{mode:'strict'}).status).toBe('blocked');expect(run(table).residuals.length).toBeGreaterThan(0);}
...
test('computed columns, table-level CHECKs and noncanonical ranges retain explicit residuals',()=>{
