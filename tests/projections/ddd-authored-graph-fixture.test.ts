...
test('FEAT-006: staged DDL and SDL fixtures share the same authored DDD graph',async()=>{
 ...
 const dddOnly=await Bun.file('fixtures/projections/ddd-graphql-entities/case.json').json();
 ...
 expect(dddOnly.logical.id).toBe(pg.logical.id);
 ...
 expect(ddd(pg.logical)).toEqual(ddd(dddOnly.logical));
