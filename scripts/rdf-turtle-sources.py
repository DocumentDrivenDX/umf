...
import rdflib
commit='369a90d1a60c021b746df2e411da0ff36258a758';root=Path('.cache/rdf-tests');folder='rdf/rdf11/rdf-turtle';dest=Path('native/rdf/turtle-sources')
...
paths=[Path('LICENSE.md'),*sorted(p.relative_to(root) for p in (root/folder).iterdir() if p.suffix in ['.ttl','.nt'] or p.name=='README')];files=[]
