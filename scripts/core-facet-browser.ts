...
try{
 ...
 await Bun.write('fixtures/validation/core-facet-candidate-browser.json',JSON.stringify({scope:'Internal candidate 0.5.0 facet schema/semantic validation and JSON/YAML metadata recovery; public document integration, migration, authoring, native bindings and admission remain pending',browser:browser.version(),checks,externalRequests,bundleSha256:createHash('sha256').update(bundle).digest('hex'),sha256},null,2)+'\n');console.log(JSON.stringify(checks));
