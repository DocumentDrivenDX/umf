...
try{
 ...
 assert.equal(checks.projectionCases,10);assert.equal(checks.projected,7);assert.equal(checks.projectionBlocks,3);assert.equal(checks.idealRecoveries,14);assert.equal(checks.nativeRecoveries,14);
