...
const stages=['discovery','constraints-native','resolved-native','projection-native','composition'];
const proofs=['discovery-native','constraints-native','resolved-native','projection-native','composition'];
const runs=[];
...
await Bun.write('fixtures/validation/facets-postgresql-native.json',JSON.stringify({scope:'PostgreSQL 17.4 native facet discovery, resolved predicates, authored DDL/value probes and composed retained recovery; full compatibility refresh required before binding acceptance',serverVersion:170004,bindingAccepted:false,idealAdmitted:false,nativeEquivalence:false,runs,sha256},null,2)+'\n');
