...
for(const row of native.cases){
 ...
 for(const [ordinal,leaf] of leaves.entries()){
  const source=parquetAvailabilitySource(bytes,leaf.index),scope=leaf.repetitionLevel?'repeated-element-value':'row-leaf-value';
  const receipt=classifyParquetNullability(source,{index:leaf.index,scope,carrier:'definition-level',mode:'strict'});
  const expected=row.leafAvailability[ordinal][leaf.repetitionLevel?'element':'row'];
  if(!receipt.target||receipt.mapping.nullability!==expected)throw Error('Native availability mismatch');
  for(const format of ['json','yaml'] as const){
   const restored=readJsonValue(writeJsonValue(copyJson(receipt),format),format) as unknown as typeof receipt,back=recoverParquetNullabilityBytes(restored,restored.target!);
   if(createHash('sha256').update(back).digest('hex')!==row.sha256)throw Error('Bytes differ');
   const path=`.cache/parquet-nullability-recovered/${row.id}-${leaf.index}-${format}.parquet`;await Bun.write(path,back);rows.push({id:row.id,index:leaf.index,scope,nullability:restored.mapping.nullability,format,path});recoveries++;
  }
 }

 files++;
}
const paths=['scripts/core-ideals/nullability-parquet-oracle.ts','fixtures/validation/nullability-parquet-native.json','src/adapters/parquet/field-metadata.ts','src/adapters/parquet/index.ts','src/adapters/parquet/footer.ts','src/adapters/parquet/schema.ts','src/model/document.ts','src/core-ideals/nullability-parquet.ts','scripts/core-ideals/nullability-parquet-cases.ts','spec/core/parquet-nullability-classification.schema.json','spec/extensions/parquet-nullability/schema.json','spec/extensions/parquet-nullability/package.json'];
