...
try{
 ...
 await Bun.write('fixtures/validation/key-postgresql-projection-native.json',JSON.stringify({scope:'Pinned generated PostgreSQL key DDL, constraint catalog and actual insertion controls; stored values only, no general input-conversion, descendant or deployed-state guarantee; full binding acceptance pending',serverVersion:170004,image:manifest.reference,references:['https://www.postgresql.org/docs/17/limits.html','https://www.postgresql.org/docs/17/datatype-numeric.html','https://www.postgresql.org/docs/17/ddl-system-columns.html'],rows,sha256},null,2)+'\n');console.log(JSON.stringify({cases:rows.length,projected:rows.filter(r=>r.status==='projected').length,probes:rows.flatMap(r=>r.probes??[]).length}));
