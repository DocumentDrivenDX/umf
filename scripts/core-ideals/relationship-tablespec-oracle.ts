...
for(const c of relationshipTableSpecProjectionCases()){
 ...
 if((c.name.startsWith('invalid-native-')||c.name.startsWith('domain-')&&c.name.endsWith('-invalid-report'))&&c.request.mode==='report'){assert.equal(r.status,'blocked');refusals.push({name:c.name,native:renderTree(getTableSpecTable(c.nativeSource))});}
