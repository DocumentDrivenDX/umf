...
fixture = 'fixtures/avro/relationship-carrier-cases.json'
...
def decode(value):
    if isinstance(value, dict):
        if set(value) == {'$bytes'}: return bytes.fromhex(value['$bytes'])
...
for case in cases:
    for writer in ['apache', 'fastavro']:
        ...
        assert row['parse']['ok'] == True, row
        if row['parse']['ok']:
            stream = io.BytesIO()
            try:
                for value in decode(case['values']):
                    if writer == 'apache':
            ...
            if row['write']['ok']:
                ...
                for reader in ['apache', 'fastavro']:
                    ...
                    assert values == decode(case['expectedValues']), (case['id'], writer, reader, values)
                    assert source.tell() == len(stream.getvalue())
                    row['reads'][reader] = {'values': encode(values), 'bytesConsumed': source.tell()}
        rows.append(row)

paths = [fixture, 'scripts/core-ideals/relationship-avro-carrier-native.py', 'scripts/core-ideals/relationship-avro-carrier-cases.ts', 'src/core-ideals/relationship-avro-carrier.ts']
proof = {'scope': 'Generated target-key tuple carriers; dangling/duplicate/empty reference values and float narrowing; no authored projection acceptance',
...
Path('fixtures/validation/relationship-avro-carrier-native.json').write_text(json.dumps(proof, indent=2)+'\n')
