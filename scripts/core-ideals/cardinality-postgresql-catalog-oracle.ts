...
try{
 ...
 assert.equal(byType.size,supplement.types.length);
 for(const c of supplement.columns)assert.ok(byType.has(identity(c.type)));
 for(const t of supplement.types)for(const edge of [t.element,t.base])if(edge)assert.ok(byType.has(identity(edge)));
 const column=(table:string,name='value')=>supplement.columns.find((c:any)=>c.schema==='cardinality'&&c.relation===table&&c.name===name);
 const type=(c:any):any=>byType.get(identity(c.type));
 assert.equal(type(column('declared')).standardArray,true);
 assert.deepEqual(type(column('declared')).element,{schema:'pg_catalog',name:'int4'});
 assert.equal(type(column('vectorish')).standardArray,false);
 assert.equal(type(column('vectorish')).category,'A');
 const wrapped=type(column('domains','vector'));assert.equal(wrapped.kind,'d');
 assert.deepEqual(wrapped.base,{schema:'pg_catalog',name:'_int4'});
 assert.equal((byType.get(identity(wrapped.base)) as any).standardArray,true);
