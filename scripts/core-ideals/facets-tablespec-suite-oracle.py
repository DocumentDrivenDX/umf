...
rows=[]
for maximum in [0,2]:rows.append({'name':'length-'+str(maximum),'data_type':'VARCHAR','expectation':'expect_column_value_lengths_to_be_between','kwargs':{'min_value':0,'max_value':maximum,'mostly':1},'stage':'raw','values':['','a','😀','e\u0301','😀😀','abc','a ',None]})
for bits,signed in [(1,True),(1,False),(8,True),(8,False),(31,False),(32,True)]:
    low=-(2**(bits-1)) if signed else 0;high=2**(bits-1)-1 if signed else 2**bits-1
    rows.append({'name':f'width-{bits}-'+('signed' if signed else 'unsigned'),'data_type':'INTEGER','expectation':'expect_column_values_to_be_between','kwargs':{'min_value':low,'max_value':high,'mostly':1},'stage':'ingested','values':list(dict.fromkeys([low-1,low,0,high,high+1]))+[None]})
for precision in [5,10]:
    bound=10**precision-1;values=[str(-bound),str(bound),'0','1.25',None]
    if precision==5:values.extend(['-100000','100000','99999.1'])
    rows.append({'name':f'decimal-{precision}-0','data_type':'DECIMAL','precision':precision,'expectation':'expect_column_values_to_be_between','kwargs':{'min_value':-bound,'max_value':bound,'mostly':1},'stage':'ingested','values':values})
...
try:
    ...
    for row in rows:
        rule={'type':row['expectation'],'kwargs':{'column':'value',**row['kwargs']},'meta':{'stage':row['stage'],'severity':'error','blocking':True,'generated_from':'user'}}
        source={'version':'1.0','table_name':'FacetSuite','columns':[{'name':'value','data_type':row['data_type'],**({'precision':row['precision'],'scale':0} if 'precision' in row else {})}],'expectations':{'expectations':[rule]}}
        ...
        namespace={};exec(compile(gen['generate_pyspark_schema'](source),'<fixed-pinned-suite-fixture>','exec'),namespace);schema=namespace['facetsuite_schema'];results=[]
