...
const rows=facetsAvroProjectionCases().filter(row=>row.request.nativeType==='float').map(row=>{const request={...row.request,obligation:'exact-input' as const};return {...row,request,expected:projectFacetsToAvro(row.author,request)};});
const targets=await Bun.file('fixtures/validation/facets-avro-projection-targets.json').json();
for(const row of rows)if(row.expected.status==='projected')assert.equal(row.expected.nativeSchema,targets.find((r:any)=>r.id===row.id).schema);
...
try{
 ...
 const checks=await page.evaluate(async(rows:any)=>{
  ...
  for(const row of rows){
   ...
   if(!r.residuals.some((x:any)=>x.reason.includes('1.0000000000000002')))throw Error('Missing narrowing residual');
   if((row.request.mode==='strict')!==(r.status==='blocked'))throw Error('Exact-input strict/report outcome');
 ...
 assert.deepEqual(checks,{cases:18,projected:9,blocked:9,idealRecoveries:18,nativeRecoveries:18,exactFacetRecoveries:9,explicitResiduals:0,forgedRejected:true,getterCalls:0});assert.deepEqual(externalRequests,[]);
 const paths=['src/index.ts','src/core-ideals/facets-avro.ts','spec/extensions/avro-facets/package.json','fixtures/validation/facets-avro-projection-targets.json',bundle,'scripts/core-ideals/facets-avro-exact-input-browser.ts','scripts/core-ideals/facets-avro-projection-cases.ts','src/core-ideals/facets-avro-projection.ts','spec/core/facets-avro-projection.schema.json'];
 const sha256=Object.fromEntries(await Promise.all(paths.map(async p=>[p,createHash('sha256').update(new Uint8Array(await Bun.file(p).arrayBuffer())).digest('hex')])));
 await Bun.write('fixtures/validation/facets-avro-exact-input-browser.json',JSON.stringify({scope:'Additional facet-gate Avro exact-input float controls; nine strict refusals and nine report candidates with retained recovery; no independent admission claim',browser:browser.version(),checks,externalRequests,sha256},null,2)+'\n');console.log(JSON.stringify(checks));
