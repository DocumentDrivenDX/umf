export {};
const schema=await Bun.file('spec/core/element-selection.schema.json').json(),core=await Bun.file('spec/core/facet-document.schema.json').json();
schema.$id='urn:umf:core:element-selection:0.5.0';schema.title='Experimental Facet metadata and explicit item/value reference selection';schema.properties.source={$ref:'urn:umf:core:0.5.0'};schema.$defs=core.$defs;
...
await Bun.write('spec/core/facet-selection.schema.json',JSON.stringify(schema,null,2)+'\n');
