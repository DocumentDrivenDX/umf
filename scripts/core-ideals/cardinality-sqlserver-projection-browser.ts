...
const fixture=await Bun.file('fixtures/validation/cardinality-sqlserver-projection-native.json').json();
const server=Bun.serve({hostname:'127.0.0.1',port:0,fetch(request){const path=new URL(request.url).pathname;
 if(path==='/umf.js')return new Response(Bun.file('dist/umf.js'),{headers:{'content-type':'text/javascript'}});
 if(path==='/cases')return Response.json(fixture);return new Response('<!doctype html><html><body>Cardinality SQL Server DDL</body></html>',{headers:{'content-type':'text/html'}});
}});
let browser;
try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  for(const row of f.rows){
   const result=await u.projectCardinalityToSqlServer(row.author,row.request);
   ...
   for(const format of ['json','yaml']){const receipt=u.readJsonValue(u.writeJsonValue(result,format),format);if(JSON.stringify(await u.recoverCardinalityFromSqlServer(receipt,result.nativeSql))!==JSON.stringify(row.author.target))throw Error('Ideal recovery');recoveries++;}
   const forged=structuredClone(result);forged.mapping.idealPath='/wrong';let refused=false;
   try{await u.recoverCardinalityFromSqlServer(forged,result.nativeSql);}catch{refused=true;}if(!refused)throw Error('Forged receipt');forgedRefusals++;
  }
  let identifiers=0;for(const value of ['x'.repeat(129),'😀'.repeat(65),'\0',String.fromCharCode(0xd800),'#temporary',' edge']){try{await u.projectCardinalityToSqlServer(f.rows[0].author,{...f.rows[0].request,tableName:value});}catch{identifiers++;continue;}throw Error('Unsafe identifier');}
