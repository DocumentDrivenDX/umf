...
for case in json.loads(Path(fixture).read_text())['cases']:
    ...
    value = {f['name']: 'account' if f['type'] == 'string' else True if f['type'] == 'boolean' else bytes.fromhex('00ff') if f['type'] == 'bytes' else 17 for f in key['fields']}
    ...
    for writer in parsed:
        ...
        for reader in parsed:
            ...
            row['reads'][reader] = {'values': portable(actual), 'bytesConsumed': source.tell()}
        rows.append(row)
