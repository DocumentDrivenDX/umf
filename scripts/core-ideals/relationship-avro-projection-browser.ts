...
try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  for(const c of cases){const r=u.projectRelationshipToAvro(c.source,c.author,c.request);if(JSON.stringify(r)!==JSON.stringify(c.expectedReceipt))throw Error('Host/browser mismatch');
   ...
   rejected=false;try{u.verifyRelationshipAvroProjection(r,u.importAvroSchema('\"string\"',{id:'stale'}));}catch{rejected=true;}if(!rejected)throw Error('Stale target accepted');staleRefusals++;
  }
