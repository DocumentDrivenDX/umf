...
try{
 ...
 const sourceSql='CREATE SCHEMA sales;\n'+await Bun.file('fixtures/postgresql/scalar-types.sql').text()+'\nCREATE SCHEMA support; CREATE TABLE support.scalar_types(id integer, note text); CREATE TABLE sales.empty_table(); CREATE TABLE sales.partitioned_events(id integer) PARTITION BY RANGE(id); CREATE VIEW sales.scalar_view AS SELECT flag FROM sales.scalar_types; CREATE TYPE sales.address AS (street text COLLATE "C", dropped integer, tags integer[]); ALTER TYPE sales.address DROP ATTRIBUTE dropped; CREATE TYPE support.address AS (id integer); CREATE TYPE sales.contact AS (address sales.address, addresses sales.address[]); CREATE TYPE sales.empty_record AS ();';await sql(sourceSql);
 ...
 const composites=[];
 for(const relation of [{schema:'sales',name:'address'},{schema:'support',name:'address'},{schema:'sales',name:'contact'},{schema:'sales',name:'empty_record'}]){
  const result=classifyPostgresqlComposite(source,{recordModule:'composites',recordId:'record',mode:'strict',nativeSource,relation});if(!result.target)throw Error('Composite blocked');
  const members=JSON.parse((await sql("SELECT coalesce(json_agg(json_build_object('name',attname,'position',attnum) ORDER BY attnum),'[]'::json) FROM pg_attribute WHERE attrelid='"+relation.schema+'.'+relation.name+"'::regclass AND attnum>0 AND NOT attisdropped")).trim());
  if(JSON.stringify(result.target.modules.at(-1)!.elements.slice(1).map(e=>e.name))!==JSON.stringify(members.map((m:any)=>m.name)))throw Error('Composite native membership');
  const kind=(await sql("SELECT relkind FROM pg_class WHERE oid='"+relation.schema+'.'+relation.name+"'::regclass")).trim();if(kind!=='c')throw Error('Not a standalone composite');
  for(const format of ['json','yaml'] as const){const receipt=readJsonValue(writeJsonValue(copyJson(result),format),format) as unknown as typeof result;if(recoverPostgresqlCompositeCapture(receipt,receipt.target!)!==nativeSource)throw Error('Composite source loss');}
  composites.push({relation,members,kind,recoveries:2});
 }
 const evidence={composites,records,viewBlocked:true,scope:'Live PostgreSQL 17.4 catalog member classification and exact capture-text recovery; table and standalone-composite record membership, no formatted-type resolution, raw DDL or native type equivalence claim',image:manifest.reference,serverVersion:version,sourceSqlSha256:createHash('sha256').update(sourceSql).digest('hex'),nativeSource,rows};
 await Bun.write('fixtures/validation/field-postgresql-native.json',JSON.stringify(evidence,null,2)+'\n');console.log(JSON.stringify({columns:rows.length,recoveries:rows.length*2,records:records.length,recordRecoveries:records.length*2,composites:composites.length,compositeRecoveries:composites.length*2,version}));
