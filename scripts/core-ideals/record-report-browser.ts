...
const fixture=await Bun.file('fixtures/validation/record-report-corpus.json').json();
...
try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  for(const row of f.rows){const result=await u.inspectRecordProjection(row.receipt,backend);if(JSON.stringify(result)!==JSON.stringify(row.result))throw Error('Common metadata parity');for(const format of ['json','yaml']){const report=u.readJsonValue(u.writeJsonValue(result,format),format);if(JSON.stringify(await u.verifyRecordProjectionInspection(report,backend))!==JSON.stringify(result))throw Error('Report round trip');recoveries++;}const altered=structuredClone(result);altered.mappings[0].basis={kind:'explicit-native-carrier',nativeType:'forged'};let rejected=false;try{await u.verifyRecordProjectionInspection(altered,backend);}catch{rejected=true;}if(!rejected)throw Error('Tampered common basis');}
  if('Bun'in globalThis||'process'in globalThis)throw Error('Host globals');return {cases:f.rows.length,recoveries};
 });
 if(external.length)throw Error('External requests');await Bun.write('fixtures/validation/record-report-browser.json',JSON.stringify({scope:'Common record receipt recomputation and metadata parity; native execution not inferred',browser:browser.version(),checks,externalRequests:external},null,2)+'\n');console.log(JSON.stringify(checks));
