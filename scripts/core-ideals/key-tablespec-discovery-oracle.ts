...
assert.equal(proof.cases.length,16);assert.equal(proof.nameBoundaries.length,16);
...
console.log(JSON.stringify({scope:proof.scope,cases:proof.cases.length,nameBoundaries:proof.nameBoundaries.length,checkedFingerprints:Object.keys(proof.sha256).length,bindingImplemented:false}));
