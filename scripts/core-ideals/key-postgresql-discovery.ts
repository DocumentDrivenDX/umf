...
const paths=['native/postgresql/catalog/image.json','native/postgresql/keys/query.sql','fixtures/postgresql/keys.sql','scripts/core-ideals/key-postgresql-discovery.ts','native/postgresql/catalog/snapshot.sql'];
...
try{
 ...
 const snapshotQuery=await Bun.file('native/postgresql/catalog/snapshot.sql').text(),snapshot=JSON.parse(await sql(snapshotQuery));
 ...
 await Bun.write('fixtures/validation/key-postgresql-catalog-capture.json',JSON.stringify(capture,null,2)+'\n');
 paths.push('fixtures/validation/key-postgresql-catalog-capture.json');
