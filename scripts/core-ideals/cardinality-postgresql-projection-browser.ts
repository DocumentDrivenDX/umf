...
const fixture=await Bun.file('fixtures/validation/cardinality-postgresql-projection-native.json').json();
const server=Bun.serve({hostname:'127.0.0.1',port:0,fetch(request){const path=new URL(request.url).pathname;
 ...
 if(path==='/cases')return Response.json(fixture);return new Response('<!doctype html><html><body>Cardinality PostgreSQL DDL</body></html>',{headers:{'content-type':'text/html'}});
}});
let browser;
try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  for(const row of f.rows){
   const result=await u.projectCardinalityToPostgresql(row.author,row.request,backend);
   ...
   for(const format of ['json','yaml']){const receipt=u.readJsonValue(u.writeJsonValue(result,format),format);if(JSON.stringify(await u.recoverCardinalityFromPostgresql(receipt,result.nativeSql,backend))!==JSON.stringify(row.author.target))throw Error('Ideal recovery');recoveries++;}
   const forged=structuredClone(result);forged.mapping.idealPath='/wrong';let refused=false;
   try{await u.recoverCardinalityFromPostgresql(forged,result.nativeSql,backend);}catch{refused=true;}if(!refused)throw Error('Forged receipt');forgedRefusals++;
  }
  let identifiers=0;for(const value of ['x'.repeat(64),'雪'.repeat(22),'\0',String.fromCharCode(0xd800)]){try{await u.projectCardinalityToPostgresql(f.rows[0].author,{...f.rows[0].request,tableName:value},backend);}catch{identifiers++;continue;}throw Error('Unsafe identifier');}
 ...
 if(external.length)throw Error('External requests');const paths=['dist/umf.js','dist/postgresql/runtime.js','dist/postgresql/libpg-query.wasm','fixtures/validation/cardinality-postgresql-projection-native.json','scripts/core-ideals/cardinality-postgresql-projection-browser.ts'];const fingerprints=Object.fromEntries(await Promise.all(paths.map(async path=>[path,createHash('sha256').update(new Uint8Array(await Bun.file(path).arrayBuffer())).digest('hex')])));await Bun.write('fixtures/validation/cardinality-postgresql-projection-browser.json',JSON.stringify({scope:'Pinned WASM syntax/AST parity and ideal recovery; native execution evidence recorded separately',fingerprints,browser:browser.version(),checks,externalRequests:external},null,2)+'\n');console.log(JSON.stringify(checks));
