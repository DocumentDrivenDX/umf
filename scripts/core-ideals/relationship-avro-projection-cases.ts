...
export function relationshipAvroCase(name='many-to-one'){
 ...
 if(name==='one-or-more')r.targetMultiplicity={min:1,max:'*'};
 ...
 if(name==='boolean')field.scalarType='boolean';
 if(name==='binary')field.scalarType='binary';
 ...
 const request:RelationshipAvroRequest={id:'avro-'+name,profile:'target-key-record',relationship:{module:'m',id:r.id},mode:'report',recordName:'Source',namespace:'example',fieldName:'reference',keyRecordName:'TargetKey',shape:name==='many-to-many'||name==='bounded-required'||name==='one-or-more'?'array':name==='nullable'?'nullable-one':'one',components:key.fields.map((f:any,i:number)=>({targetField:f,name:'key_'+i,type:logical.modules[0].elements.find((e:any)=>e.id===f.element).scalarType==='string'?'string':name==='boolean'?'boolean':name==='binary'?'bytes':name==='int32'?'int':'long'}))};
 ...
 if(name==='duplicate-components')request.components.push({...request.components[0]!});
 if(name==='reserved-name')request.recordName='long';
...
export function relationshipAvroProjectionCases(){
 const blocked=new Set(['heterogeneous','heterogeneous-target','association','wrong-key-field','type-conflict','invalid-name','duplicate-components','reserved-name']);
 return ['one-or-more','boolean','binary','int32','duplicate-components','reserved-name','many-to-one','one-to-one','many-to-many','bounded-required','owned','undirected','heterogeneous','heterogeneous-target','association','self','alternate','composite','unknown','nullable','facet','wrong-key-field','type-conflict','invalid-name'].flatMap(name=>(['strict','report'] as const).map(mode=>{const c=relationshipAvroCase(name);c.request.mode=mode;return {...c,name:name+'-'+mode,expected:mode==='strict'||blocked.has(name)?'blocked' as const:'projected' as const};}));
}
if(import.meta.main){
