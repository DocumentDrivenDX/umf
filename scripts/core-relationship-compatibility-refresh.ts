...
const output='fixtures/validation/relationship-compatibility-refresh.json',directory='.cache/relationship-compatibility-refresh';
await mkdir(directory,{recursive:true});
const sourcePaths=(await Promise.all(['src/**/*.ts','scripts/**/*.ts','scripts/**/*.py','spec/**/*.json'].map(p=>Array.fromAsync(new Bun.Glob(p).scan())))).flat().sort();
...
const record=(complete:boolean)=>({scope:'Fresh compatibility command replay for public 0.7.0; separate regression and core-task acceptance required',complete,coreTaskAccepted:false,idealAdmitted:false,nativeEquivalence:false,commands,runs,sha256});
