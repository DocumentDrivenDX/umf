...
const report=await Bun.file('fixtures/typespec/library-corpus-results.json').json();
...
try{
 ...
 await Bun.write('fixtures/typespec/library-corpus-oracle-results.json',JSON.stringify({libraries:report.libraries,compiler:report.compiler,programs:rows,scope:'Filesystem host versus memory host, supplied .tsp files only; selected pinned libraries enabled; other configuration/custom JS context remains archived. Same compiler implementation.'},null,2)+'\n');
 console.log('TypeSpec library corpus: all '+rows.length+' filesystem-host outcomes agree; '+rows.filter(x=>x.valid).length+' compile with current libraries');
