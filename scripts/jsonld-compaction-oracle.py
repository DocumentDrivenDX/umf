...
base=Path('fixtures/jsonld/compaction');results=[]
...
for c in json.loads((base/'results.json').read_text())['cases']:
    ...
    options['compactToRelative']=c['compactOptions']['compactToRelative']
    options['compactArrays']=c['compactOptions']['compactArrays']
    context=processor_input(c['compactOptions']['context']) if 'context' in c['compactOptions'] else None
    try:expanded=jsonld.compact(processor_input(Path(c['path']).read_text()),context,options)
