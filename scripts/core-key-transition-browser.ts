...
try{
 ...
 await Bun.write('fixtures/validation/core-key-transition-browser.json',JSON.stringify({scope:'Explicit experimental Key 0.5.0-to-0.6.0 migration/rollback through a focused browser bundle; public entry-point integration is checked separately, with no native binding or author-provenance claim',browser:browser.version(),checks,externalRequests,bundleSha256:createHash('sha256').update(bundle).digest('hex'),sha256},null,2)+'\n');
