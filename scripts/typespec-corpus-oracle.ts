...
const report=await Bun.file('fixtures/typespec/corpus-results.json').json();
...
try{
 ...
 await Bun.write('fixtures/typespec/corpus-oracle-results.json',JSON.stringify({commit:report.commit,compiler:report.compiler,programs:rows,scope:'Filesystem host versus memory host, supplied .tsp files only; configuration/JS context is archived but not enabled. Same compiler implementation.'},null,2)+'\n');
 console.log('TypeSpec upstream corpus: all '+rows.length+' filesystem-host outcomes agree; '+rows.filter(x=>x.valid).length+' compile with current libraries');
