...
base=Path('fixtures/jsonld/compaction/authored');results=[]
for c in json.loads((base/'results.json').read_text())['results']:
    ...
    options={'compactToRelative':c['options'].get('compactToRelative',True),'base':inputs['baseIRI'],'documentLoader':loader,'compactArrays':c['options'].get('compactArrays',True)}
    ...
    try:
        output=jsonld.compact(processor_input(c['source']),ctx,options)
        ...
        if c.get('edit'):
            ...
            edited=jsonld.compact(source,ctx,options)
