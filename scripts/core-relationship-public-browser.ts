...
import {selectCoreRelationships} from '../src/model/relationship-selection';
...
const cases=rows.map(row=>{const declaration=declareCoreRelationship(row.source,row.identity,row.request);return {...row,declaration,selection:selectCoreRelationships(declaration.target,{}),inspection:inspectCoreRelationships(declaration.target,row.identity),lookup:lookupCoreRelationship(declaration.target,{...row.identity,id:row.request.id})};});
...
try{
 ...
 const checks=await page.evaluate(async()=>{
  const path='/candidate.js',u=await import(path),cases=await(await fetch('/cases')).json();let recoveries=0,refusals=0,selectionRecoveries=0;
  ...
  for(const row of cases){
   ...
   const relationshipSelection=u.selectCoreRelationships(r.target,{});same(relationshipSelection,row.selection);
   for(const format of ['json','yaml']){same(u.verifyCoreRelationshipSelection(u.readJsonValue(u.writeJsonValue(relationshipSelection,format),format)),relationshipSelection);selectionRecoveries++;}
  ...
  if(relationships.selection.length!==1||relationships.selection[0].targets[0].reference.key!=='identity')throw Error('Relationship selection mismatch');
  for(const format of ['json','yaml'])same(u.verifyCoreRelationshipSelection(u.readJsonValue(u.writeJsonValue(relationships,format),format)),relationships);
  const fakeSelection=structuredClone(relationships);fakeSelection.selection[0].targets[0].keyPath='/forged';refuses(()=>u.verifyCoreRelationshipSelection(fakeSelection));
  refuses(()=>u.declareCoreNullability(doc,field,'absent-allowed'));refuses(()=>u.declareCoreElementKind(doc,record,'group'));
  if(getterCalls||'Bun'in globalThis||'process'in globalThis)throw Error('Host or getter behavior leaked');return {cases:cases.length,recoveries,selectionRecoveries,refusals,getterCalls};
 });
 assert.equal(checks.cases,cases.length);assert.equal(checks.selectionRecoveries,cases.length*2);assert.equal(checks.recoveries,cases.length*6);assert.equal(checks.refusals,cases.length*3+5);assert.deepEqual(externalRequests,[]);
 const paths=['scripts/core-relationship-public-browser.ts','src/index.ts','src/model/types.ts','src/model/field-kind.ts','src/model/record-type.ts','src/model/nullability.ts','src/model/cardinality.ts','src/model/facets.ts','src/model/keys.ts','src/model/key-tuple.ts','src/model/selection.ts','src/model/relationship-selection.ts','scripts/core-relationship-metadata-schema.ts','spec/core/relationship-metadata-selection.schema.json','tests/core/relationship-selection.test.ts','src/model/selection-verification.ts','tests/core/relationship-public.test.ts','scripts/core-relationship-versioned-schemas.ts','dist/umf.js','scripts/core-relationship-operation-cases.ts','scripts/core-relationship-operation-schema.ts','scripts/core-relationship-cases.ts','src/model/relationships.ts','src/validation/relationships.ts','src/validation/keys.ts','src/validation/document.ts','src/model/json.ts','src/model/serialization.ts','spec/core/relationship-document.schema.json','spec/core/relationship-operation.schema.json','tests/core/relationship-operations.test.ts'];
