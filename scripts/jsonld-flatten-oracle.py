...
base=Path('fixtures/jsonld/flatten');results=[]
...
for c in json.loads((base/'results.json').read_text())['cases']:
    ...
    options['compactArrays']=c['flattenOptions']['compactArrays']
    context=processor_input(c['flattenOptions']['context']) if 'context' in c['flattenOptions'] else None
    try:expanded=jsonld.flatten(processor_input(Path(c['path']).read_text()),context,options)
