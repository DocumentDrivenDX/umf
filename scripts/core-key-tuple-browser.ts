...
try{
 ...
 await Bun.write('fixtures/validation/core-key-tuple-browser.json',JSON.stringify({scope:'Internal candidate exact Key tuple encoding and receipt verification; no native binding or author-provenance claim',browser:browser.version(),checks,externalRequests,bundleSha256:createHash('sha256').update(bundle).digest('hex'),sha256},null,2)+'\n');
