...
rows=[]
for c in json.loads(Path('fixtures/shacl/string-results.json').read_text())['cases']:
 try:
...
Path('fixtures/shacl/string-oracle.json').write_text(json.dumps({'oracle':'PySHACL','version':pyshacl.__version__,'rdflib':rdflib.__version__,'results':rows},indent=2)+'\n');print({'cases':len(rows),'equal':sum(r['equal'] for r in rows),'differences':[r for r in rows if not r['equal']]})
