"""Pin every JSON file under a delta-rs _delta_log directory, including error fixtures."""
import concurrent.futures,hashlib,json,urllib.request
from pathlib import Path
commit='90b904ede68627c2450007034d9724c043f1a66b';base=Path('fixtures/delta/upstream');base.mkdir(exist_ok=True)
...
files=[x for x in tree['tree'] if x['type']=='blob' and ('/_delta_log/' in x['path'] and x['path'].endswith('.json') or x['path']=='LICENSE.txt')];assert len(files)==308,len(files)
def fetch(entry):
...
(base/'manifest.json').write_text(json.dumps({'repository':'https://github.com/delta-io/delta-rs','commit':commit,'selection':'Every JSON blob under an _delta_log directory plus LICENSE.txt; excludes data/checkpoint files','files':results},indent=2)+'\n');print({'files':len(results),'bytes':sum(x['size'] for x in results)})
