...
const entry='.cache/core-relationship-operations-browser-entry.ts';
await Bun.write(entry,"export * from '../src/model/relationships';export {readJsonValue,writeJsonValue} from '../src/model/serialization';\n");
...
try{
 ...
 assert.equal(checks.cases,cases.length);assert.equal(checks.recoveries,cases.length*6);assert.equal(checks.refusals,cases.length*3+2);assert.deepEqual(externalRequests,[]);
 const paths=['scripts/core-relationship-operations-browser.ts','scripts/core-relationship-operation-cases.ts','scripts/core-relationship-operation-schema.ts','scripts/core-relationship-cases.ts','src/model/relationships.ts','src/validation/relationships.ts','src/validation/keys.ts','src/validation/document.ts','src/model/json.ts','src/model/serialization.ts','spec/core/relationship-document.schema.json','spec/core/relationship-operation.schema.json','tests/core/relationship-operations.test.ts'];
 const sha256=Object.fromEntries(await Promise.all(paths.map(async p=>[p,createHash('sha256').update(new Uint8Array(await Bun.file(p).arrayBuffer())).digest('hex')])));
 await Bun.write('fixtures/validation/core-relationship-operations-browser.json',JSON.stringify({scope:'Experimental relationship authoring, inspection and lookup; public version integration and native binding admission remain separate',browser:browser.version(),checks,externalRequests,bundleSha256:createHash('sha256').update(bundle).digest('hex'),sha256},null,2)+'\n');console.log(JSON.stringify(checks));
