...
from jsonld_oracle_support import exact,processor_input,equivalent
base=Path('fixtures/jsonld/flatten/authored');results=[]
for c in json.loads((base/'results.json').read_text())['results']:
    ...
    options={'base':inputs['baseIRI'],'documentLoader':loader,'compactArrays':c['options'].get('compactArrays',True)}
    if 'expandContext' in inputs:options['expandContext']=processor_input(inputs['expandContext'])
    ctx=processor_input(c['options']['context']) if 'context' in c['options'] else None
    row={'id':c['id'],'umf':c['status'],'lossPolicy':c['options']['lossPolicy']}
    try:
        output=jsonld.flatten(processor_input(c['source']),ctx,options)
        ...
        if c.get('edit'):
            ...
            source=processor_input(c['source']);source['urn:p'][1]=processor_input(c['edit']['text'])
            edited=jsonld.flatten(source,ctx,options)
