...
const server=Bun.serve({hostname:'127.0.0.1',port:0,fetch(request){const path=new URL(request.url).pathname;
  ...
  return new Response('<!doctype html><html>DDD GraphQL core Field projection</html>',{headers:{'content-type':'text/html'}});
}});
let browser;
try{
  ...
  await Bun.write('fixtures/projections/ddd-graphql-core-fields/browser.json',JSON.stringify({browser:browser.version(),result,externalRequests:external},null,2)+'\n');
