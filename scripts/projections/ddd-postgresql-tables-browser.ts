...
import {projectDddTablesToPostgresql,type Document} from '../../src';

const fixture=await Bun.file('fixtures/projections/ddd-postgresql-tables/case.json').json();
const expected=await projectDddTablesToPostgresql(fixture.logical as Document,fixture.binding as Document,backend,fixture.policy,'report');
const server=Bun.serve({hostname:'127.0.0.1',port:0,fetch(request){const path=new URL(request.url).pathname;
  ...
  return new Response('<!doctype html><html>DDD PostgreSQL tables</html>',{headers:{'content-type':'text/html'}});
}});
let browser;
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const report=await u.projectDddTablesToPostgresql(fixture.logical,fixture.binding,backend,fixture.policy,'report');
