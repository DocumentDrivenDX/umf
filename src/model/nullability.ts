...
export interface CoreNullabilityInspection {operation:'inspect-core-nullability';version:'1.0.0'|'2.0.0';source:Document;identity:CoreNullabilityIdentity;path:string;meaning:CoreNullabilityMeaning;provenance:'unverified'}
export interface CoreNullabilityDeclaration {operation:'declare-core-nullability';version:'1.0.0'|'2.0.0';source:Document;target:Document;identity:CoreNullabilityIdentity;provenance:{origin:'authored';idealPath:string;nullability:Nullability;binding:{id:'umf.core.nullability.authoring';version:'1.0.0'|'2.0.0'};basis:'explicit-author-declaration';nativePath:null}}
const validator=createValidator();validator.addSchema(legacy);validator.addSchema(fields);validator.addSchema(availability);validator.addSchema(containers);const checkV1=validator.compile(schema),checkV2=validator.compile(schemaV2);
...
export function inspectCoreNullability(input:Document,identity:CoreNullabilityIdentity):CoreNullabilityInspection {
 ...
 if(Object.hasOwn(element,'nullability')){
  if(source.umf!=='0.3.0'&&source.umf!=='0.4.0')meaning={state:'legacy',value:copyJson(element.nullability)};
 ...
 }else if((source.umf==='0.3.0'||source.umf==='0.4.0')&&element.kind!=='field')meaning={state:'inapplicable'};
 return finish({operation:'inspect-core-nullability',version:source.umf==='0.4.0'?'2.0.0':'1.0.0',source,identity:located.identity,path,meaning,provenance:'unverified'});
}
...
export function declareCoreNullability(input:Document,identity:CoreNullabilityIdentity,nullability:Nullability):CoreNullabilityDeclaration {
 ...
 if(source.umf!=='0.3.0'&&source.umf!=='0.4.0')throw new UmfError('CORE_NULLABILITY_VERSION','Explicit envelope migration is required before authoring nullability');
 ...
 return finish({operation:'declare-core-nullability',version:source.umf==='0.4.0'?'2.0.0':'1.0.0',source,target,identity:located.identity,provenance:{origin:'authored',idealPath:path,nullability,binding:{id:'umf.core.nullability.authoring',version:source.umf==='0.4.0'?'2.0.0':'1.0.0'},basis:'explicit-author-declaration',nativePath:null}});
}
