...
export interface CoreNullabilityInspection {operation:'inspect-core-nullability';version:'1.0.0';source:Document;identity:CoreNullabilityIdentity;path:string;meaning:CoreNullabilityMeaning;provenance:'unverified'}
export interface CoreNullabilityDeclaration {operation:'declare-core-nullability';version:'1.0.0';source:Document;target:Document;identity:CoreNullabilityIdentity;provenance:{origin:'authored';idealPath:string;nullability:Nullability;binding:{id:'umf.core.nullability.authoring';version:'1.0.0'};basis:'explicit-author-declaration';nativePath:null}}
const validator=createValidator();validator.addSchema(legacy);validator.addSchema(fields);validator.addSchema(availability);const check=validator.compile(schema);
function finish<T>(value:T):T {const result=copyJson(value);if(!check(result))throw new UmfError('CORE_NULLABILITY_RESULT',JSON.stringify(check.errors));return result as T;}
...
export function inspectCoreNullability(input:Document,identity:CoreNullabilityIdentity):CoreNullabilityInspection {
 ...
 if(Object.hasOwn(element,'nullability')){
  if(source.umf!=='0.3.0')meaning={state:'legacy',value:copyJson(element.nullability)};
 ...
 }else if(source.umf==='0.3.0'&&element.kind!=='field')meaning={state:'inapplicable'};
 return finish({operation:'inspect-core-nullability',version:'1.0.0',source,identity:located.identity,path,meaning,provenance:'unverified'});
}
...
export function declareCoreNullability(input:Document,identity:CoreNullabilityIdentity,nullability:Nullability):CoreNullabilityDeclaration {
 ...
 if(source.umf!=='0.3.0')throw new UmfError('CORE_NULLABILITY_VERSION','Explicit envelope migration is required before authoring nullability');
 ...
 return finish({operation:'declare-core-nullability',version:'1.0.0',source,target,identity:located.identity,provenance:{origin:'authored',idealPath:path,nullability,binding:{id:'umf.core.nullability.authoring',version:'1.0.0'},basis:'explicit-author-declaration',nativePath:null}});
}
...
export function verifyCoreNullabilityDeclaration(input:CoreNullabilityDeclaration,current:Document):CoreNullabilityDeclaration {
 ...
 if(!check(receipt)||receipt.operation!=='declare-core-nullability')throw new UmfError('CORE_NULLABILITY_RECEIPT','Malformed nullability declaration');
