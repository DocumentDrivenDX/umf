...
interface Declaration {version:'1.0.0';source:Document;target:Document;identity:CoreRecordIdentity;provenance:{origin:'authored';idealPath:string;basis:'explicit-author-declaration';nativePath:null;binding:{id:string;version:'1.0.0'}}}
...
export interface CoreKeyInspection {operation:'inspect-core-keys';version:'1.0.0';source:Document;identity:CoreRecordIdentity;path:string;meaning:CoreKeyMeaning;provenance:'unverified'}
export interface CoreKeyLookup {operation:'lookup-core-key';version:'1.0.0';source:Document;identity:CoreKeyIdentity;path:string;key:CoreKeyDefinition;uninterpretedPaths:string[];provenance:'unverified'}
export type CoreKeyOperation=CoreKeyDeclaration|CoreRecordMembersDeclaration|CoreKeyInspection|CoreKeyLookup;
const validator=createValidator();for(const s of [legacy,fields,availability,containers,facets,keys])validator.addSchema(s);
const check=validator.compile(schema),identityCheck=validator.compile(schema.$defs.identity),keyIdentityCheck=validator.compile(schema.$defs.keyIdentity),requestCheck=validator.compile(schema.$defs.keyRequest),membersCheck=validator.compile(schema.$defs.memberRequest);
...
function finish<T extends CoreKeyOperation>(result:T):T{const copied=copyJson(result);if(!check(copied))throw new UmfError('CORE_KEY_RESULT',JSON.stringify(check.errors));return copied as unknown as T;}
...
function record(located:ReturnType<typeof locate>){
 if(located.source.umf!=='0.6.0')throw new UmfError('CORE_KEY_VERSION','Explicit Key envelope migration is required');
...
export function inspectCoreKeys(input:Document,identity:CoreRecordIdentity):CoreKeyInspection{
 ...
 if(Object.hasOwn(l.element,'keys')){
  if(l.source.umf!=='0.6.0')meaning={state:'legacy',value:copyJson(l.element.keys)};
 ...
 }else if(l.source.umf==='0.6.0'&&l.element.kind!=='record')meaning={state:'inapplicable'};
 return finish({operation:'inspect-core-keys',version:'1.0.0',source:l.source,identity:l.identity,path,meaning,provenance:'unverified'});
}
export function lookupCoreKey(input:Document,identityInput:CoreKeyIdentity):CoreKeyLookup{
 ...
 return finish({operation:'lookup-core-key',version:'1.0.0',source:l.source,identity,path,key:values![index]!,uninterpretedPaths:unknown(l.validation.diagnostics,path),provenance:'unverified'});
}
export function declareCoreRecordMembers(input:Document,identity:CoreRecordIdentity,requestInput:CoreRecordIdentity[]):CoreRecordMembersDeclaration{
 ...
 return finish({operation:'declare-core-record-members',version:'1.0.0',source:l.source,target,identity:l.identity,request,provenance:{origin:'authored',idealPath:l.path+'/members',basis:'explicit-author-declaration',nativePath:null,binding:{id:'umf.core.members.authoring',version:'1.0.0'}}});
}
export function declareCoreKey(input:Document,identity:CoreRecordIdentity,requestInput:CoreKeyRequest):CoreKeyDeclaration{
 ...
 return finish({operation:'declare-core-key',version:'1.0.0',source:l.source,target,identity:l.identity,request,provenance:{origin:'authored',idealPath:l.path+`/keys/${index<0?old.length:index}`,basis:'explicit-author-declaration',nativePath:null,binding:{id:'umf.core.key.authoring',version:'1.0.0'}}});
}
...
export function verifyCoreKeyOperation(input:CoreKeyOperation,current:Document):CoreKeyOperation{
 const receipt=copyJson(input) as unknown as CoreKeyOperation;if(!check(receipt))throw new UmfError('CORE_KEY_RECEIPT','Malformed key operation');
