...
export interface CoreCardinalityInspection {operation:'inspect-core-cardinality';version:'1.0.0';source:Document;identity:CoreCardinalityIdentity;path:string;meaning:CoreCardinalityMeaning;provenance:'unverified'}
export interface CoreCardinalityRequest {cardinality:Cardinality;itemType?:CoreItemTypeReference|null}
export interface CoreCardinalityDeclaration {operation:'declare-core-cardinality';version:'1.0.0';source:Document;target:Document;identity:CoreCardinalityIdentity;request:CoreCardinalityRequest;provenance:{origin:'authored';idealPath:string;cardinality:Cardinality;binding:{id:'umf.core.cardinality.authoring';version:'1.0.0'};basis:'explicit-author-declaration';nativePath:null}}
const validator=createValidator();validator.addSchema(legacy);validator.addSchema(fields);validator.addSchema(availability);validator.addSchema(containers);const check=validator.compile(schema);const checkRequest=validator.compile({$defs:schema.$defs,$ref:"#/$defs/request"});
function finish<T>(value:T):T {const result=copyJson(value);if(!check(result))throw new UmfError('CORE_CARDINALITY_RESULT',JSON.stringify(check.errors));return result as T;}
...
export function inspectCoreCardinality(input:Document,identity:CoreCardinalityIdentity):CoreCardinalityInspection {
 ...
 if(Object.hasOwn(element,'cardinality')){
  if(source.umf!=='0.4.0')meaning={state:'legacy',value:copyJson(element.cardinality)};
 ...
 }else if(source.umf==='0.4.0'&&element.kind!=='field')meaning={state:'inapplicable'};
 return finish({operation:'inspect-core-cardinality',version:'1.0.0',source,identity:located.identity,path,meaning,provenance:'unverified'});
}
...
export function declareCoreCardinality(input:Document,identity:CoreCardinalityIdentity,options:CoreCardinalityRequest):CoreCardinalityDeclaration {
 ...
 if(source.umf!=='0.4.0')throw new UmfError('CORE_CARDINALITY_VERSION','Explicit envelope migration is required before authoring cardinality');
 ...
 return finish({operation:'declare-core-cardinality',version:'1.0.0',source,target,identity:located.identity,request,provenance:{origin:'authored',idealPath:path,cardinality,binding:{id:'umf.core.cardinality.authoring',version:'1.0.0'},basis:'explicit-author-declaration',nativePath:null}});
}
...
export function verifyCoreCardinalityDeclaration(input:CoreCardinalityDeclaration,current:Document):CoreCardinalityDeclaration {
 ...
 if(!check(receipt)||receipt.operation!=='declare-core-cardinality')throw new UmfError('CORE_CARDINALITY_RECEIPT','Malformed cardinality declaration');
