...
import schemaV2 from '../../spec/core/facet-operation-v2.schema.json';
...
export interface CoreFacetInspection {operation:'inspect-core-facets';version:'1.0.0'|'2.0.0';source:Document;identity:CoreFacetIdentity;path:string;meaning:CoreFacetMeaning;provenance:'unverified'}
export interface CoreFacetDeclaration {operation:'declare-core-facets';version:'1.0.0'|'2.0.0';source:Document;target:Document;identity:CoreFacetIdentity;request:CoreFacetPatch;provenance:{origin:'authored';idealPath:string;binding:{id:'umf.core.facets.authoring';version:'1.0.0'|'2.0.0'};basis:'explicit-author-declaration';nativePath:null}}
const validator=createValidator();for(const s of [legacy,fields,availability,containers,facets,keys])validator.addSchema(s);
const checkV1=validator.compile(schema),checkV2=validator.compile(schemaV2);const checker=(version:unknown)=>version==='2.0.0'?checkV2:checkV1;const checkRequest=validator.compile({$defs:schema.$defs,$ref:'#/$defs/request'});
...
export function inspectCoreFacets(input:Document,identity:CoreFacetIdentity):CoreFacetInspection {
 ...
 if(Object.hasOwn(element,'facets')){
  if(source.umf!=='0.5.0'&&source.umf!=='0.6.0')meaning={state:'legacy',value:copyJson(element.facets)};
  else{
 ...
 }else if((source.umf==='0.5.0'||source.umf==='0.6.0')&&element.kind!=='field')meaning={state:'inapplicable'};
 return finish({operation:'inspect-core-facets',version:source.umf==='0.6.0'?'2.0.0':'1.0.0',source,identity:located.identity,path,meaning,provenance:'unverified'});
}
...
export function declareCoreFacets(input:Document,identity:CoreFacetIdentity,options:CoreFacetPatch):CoreFacetDeclaration {
 ...
 if(source.umf!=='0.5.0'&&source.umf!=='0.6.0')throw new UmfError('CORE_FACET_VERSION','Explicit envelope migration is required before authoring facets');
 ...
 return finish({operation:'declare-core-facets',version:source.umf==='0.6.0'?'2.0.0':'1.0.0',source,target,identity:located.identity,request,provenance:{origin:'authored',idealPath:path,binding:{id:'umf.core.facets.authoring',version:source.umf==='0.6.0'?'2.0.0':'1.0.0'},basis:'explicit-author-declaration',nativePath:null}});
}
...
export function verifyCoreFacetDeclaration(input:CoreFacetDeclaration,current:Document):CoreFacetDeclaration {
 ...
 if(!checker(receipt?.version)(receipt)||receipt.operation!=='declare-core-facets')throw new UmfError('CORE_FACET_RECEIPT','Malformed facet declaration');
