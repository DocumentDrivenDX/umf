...
import schema from '../../spec/core/nullability-parquet-projection.schema.json';
export {default as nullabilityParquetProjectionSchema} from '../../spec/core/nullability-parquet-projection.schema.json';
export interface NullabilityParquetRequest {id:string;recordName:string;fieldName:string;nativeType:keyof typeof parquetCarriers;mode:'strict'|'report';scope:'row-leaf-value'|'repeated-element-value'|'write-input'|'unresolved';carrier:'definition-level'|'unresolved'}
const binding=schema.properties.binding.const;
export interface NullabilityParquetProjection {
 operation:'project-nullability-parquet';version:'1.0.0';status:'projected'|'blocked';source:Document;author:CoreNullabilityDeclaration;request:NullabilityParquetRequest;target?:Document;binding:typeof binding;diagnostics:Diagnostic[];
 mapping:{origin:'authored';nullability:'required'|'absent-allowed'|'unspecified';encoding:'required'|'optional';basis:'authored-requirement'|'no-authored-requirement'|'unprojected-requirement';scope:NullabilityParquetRequest['scope'];carrier:NullabilityParquetRequest['carrier'];idealPath:string;nativePath:'/schema/1/repetition_type';outcome:'exact'|'unknown'|'not-expressible'};
...
export function projectNullabilityToParquet(input:CoreNullabilityDeclaration,options:NullabilityParquetRequest):NullabilityParquetProjection & {diagnostics:Diagnostic[]} {
 const copied=copyJson(input) as unknown as CoreNullabilityDeclaration;
 const author=verifyCoreNullabilityDeclaration(copied,copied.target),source=copyJson(author.target) as unknown as Document,request=copyJson(options) as unknown as NullabilityParquetRequest;
 if(!checkRequest(request))throw new UmfError('NULLABILITY_PARQUET_REQUEST',JSON.stringify(checkRequest.errors));
 ...
 const result:NullabilityParquetProjection={operation:'project-nullability-parquet',version:'1.0.0',status:'projected',source,author,request,binding,mapping:{origin:'authored',nullability:author.provenance.nullability,encoding:'optional',basis:author.provenance.nullability==='unspecified'?'no-authored-requirement':'authored-requirement',scope:request.scope,carrier:request.carrier,idealPath:path+'/nullability',nativePath:'/schema/1/repetition_type',outcome:'exact'},residuals:[],diagnostics:[]};
 ...
 for(const [key,value] of Object.entries(element))if(!['id','kind','name','scalarType','nullability'].includes(key)&&!(key==='extensions'&&value&&typeof value==='object'&&Object.keys(value).length===0))loss(path+'/'+pointer(key),value,'Field metadata is retained but not projected');
 if(author.provenance.nullability!=='unspecified'){
  if(request.scope!=='row-leaf-value'){loss(path+'/nullability',author.provenance.nullability,'This flat native schema implements row-leaf-value scope only; repeated entries and writer inputs require other bindings','not-expressible');result.mapping.basis='unprojected-requirement';}
  else if(request.carrier!=='definition-level'){loss(path+'/nullability',author.provenance.nullability,'Explicit physical definition-level carrier required; no Arrow-mask or input-omission equivalence is inferred');result.mapping.basis='unprojected-requirement';}
  else if(author.provenance.nullability==='required')result.mapping.encoding='required';
 }
 const target=importParquetSchema(parquetFieldFile(request.recordName,request.fieldName,request.nativeType,result.mapping.encoding),{id:request.id});
 ...
 const output=copyJson(result);if(!check(output))throw new UmfError('NULLABILITY_PARQUET_RESULT',JSON.stringify(check.errors));return output as unknown as NullabilityParquetProjection & {diagnostics:Diagnostic[]};
}
/** Recover retained author meaning only when the receipt and supplied native target still agree. */
export function recoverNullabilityFromParquet(input:NullabilityParquetProjection,nativeBytes:Uint8Array):Document {
 const receipt=copyJson(input) as unknown as NullabilityParquetProjection;
 if(!check(receipt)||receipt.status!=='projected'||!receipt.target)throw new UmfError('NULLABILITY_PARQUET_RECEIPT','Expected projected receipt');
 const expected:NullabilityParquetProjection=projectNullabilityToParquet(receipt.author,receipt.request);
 ...
 if(canonical(copyJson(expected))!==canonical(copyJson(receipt)))throw new UmfError('NULLABILITY_PARQUET_RECEIPT','Receipt does not match retained source and binding');
 const expectedBytes=exportParquetCapture(receipt.target);
 if(!(nativeBytes instanceof Uint8Array)||nativeBytes.length!==expectedBytes.length||nativeBytes.some((b,i)=>b!==expectedBytes[i]))throw new UmfError('NULLABILITY_PARQUET_STALE','Native target changed; retained report cannot assert recovery');
