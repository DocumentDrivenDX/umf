...
export function parquetFieldFile(recordName:string,fieldName:string,nativeType:keyof typeof parquetCarriers,repetition:'required'|'optional'|'repeated'):Uint8Array{
 ...
 const carrier=parquetCarriers[nativeType],root=struct([{id:4,value:text(recordName)},{id:5,value:int(1)}]),field=struct([{id:1,value:int(carrier.type)},{id:3,value:int({required:0,optional:1,repeated:2}[repetition])},{id:4,value:text(fieldName)},...('converted'in carrier?[{id:6,value:int(carrier.converted)}]:[])]);
 const footer=encodeParquetWire(struct([{id:1,value:int(1)},{id:2,value:{kind:'list',elementType:'struct',items:[root,field]}},{id:3,value:{kind:'i64',value:'0'}},{id:4,value:{kind:'list',elementType:'struct',items:[]}}]));
