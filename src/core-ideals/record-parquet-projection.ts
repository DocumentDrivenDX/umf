import {parquetRecordFile} from './parquet-carriers';
...
import legacy from '../../spec/core/schema.json';import fields from '../../spec/core/field-document.schema.json';import kinds from '../../spec/core/kind-operation.schema.json';import schema from '../../spec/core/record-parquet-projection.schema.json';
export {default as recordParquetProjectionSchema} from '../../spec/core/record-parquet-projection.schema.json';
export interface RecordParquetRequest {id:string;recordName:string;mode:'strict'|'report';fields:{author:CoreKindDeclaration;fieldName:string;nativeType:FieldParquetRequest['nativeType'];repetition:FieldParquetRequest['repetition']}[]}
const binding={id:'umf.core.record.parquet',version:'1.0.0',nativeVersion:'parquet-format@219e3f12a62f9476e830c21e26d030d231f7c017',subset:'Authored flat record with explicit field carriers/repetition in an empty schema file; no row writing or value-domain equivalence'} as const;
export interface RecordParquetProjection {operation:'project-record-parquet';version:'1.0.0';status:'projected'|'blocked';source:Document;author:CoreKindDeclaration;request:RecordParquetRequest;binding:typeof binding;target?:Document;mappings:{origin:'authored';kind:'record'|'field';idealPath:string;nativePath:string;outcome:'exact'|'unknown'|'not-expressible'}[];residuals:FieldParquetProjection['residuals'];diagnostics:Diagnostic[]}
...
export function projectRecordToParquet(input:CoreKindDeclaration,options:RecordParquetRequest):RecordParquetProjection {
 const copied=copyJson(input) as unknown as CoreKindDeclaration,author=verifyCoreKindDeclaration(copied,copied.target),source=copyJson(author.target) as unknown as Document,request=copyJson(options) as unknown as RecordParquetRequest;
 if(!checkRequest(request))throw new UmfError('RECORD_PARQUET_REQUEST',JSON.stringify(checkRequest.errors));
 const locate=(id:{module:string;element:string})=>{const mi=source.modules.findIndex(m=>m.id===id.module),ei=source.modules[mi]?.elements.findIndex(e=>e.id===id.element)??-1;if(mi<0||ei<0)throw new UmfError('RECORD_PARQUET_IDENTITY','Missing declared element');return {element:source.modules[mi]!.elements[ei]!,path:`/modules/${mi}/elements/${ei}`,mi};};
 ...
 const result:RecordParquetProjection={operation:'project-record-parquet',version:'1.0.0',status:'projected',source,author,request,binding,mappings:[],residuals:[],diagnostics:[]};let hard=false;
 ...
 const bindings=new Map<string,RecordParquetRequest['fields'][number]>(),names=new Set<string>();
 ...
 const columns:Parameters<typeof parquetRecordFile>[1]=[],included=new Set([selected.path]),locations:{path:string;scalarType?:string}[]=[];
 for(const [key,ref] of members){
  ...
  result.mappings.push({origin:'authored',kind:'field',idealPath:found.path+'/kind',nativePath:'/schema/'+(columns.length+1),outcome:'exact'});
  columns.push({fieldName:field.fieldName,nativeType:field.nativeType,repetition:field.repetition});locations.push({path:found.path,...(found.element.scalarType!==undefined?{scalarType:found.element.scalarType}:{})});
 }
 ...
 source.modules.forEach((module,mi)=>{
  ...
  if(module.namespace)loss(root+'/namespace',module.namespace,'Parquet root name does not preserve ideal namespace');
  ...
  module.elements.forEach((element,ei)=>{
   ...
   const known=path===selected.path?['id','kind','name','references']:['id','kind','name','scalarType'];
 ...
 if(!hard){
  target=importParquetSchema(parquetRecordFile(request.recordName,columns),{id:request.id});
  locations.forEach((location,index)=>{if(location.scalarType!==undefined&&location.scalarType!==getParquetFieldMetadata(target!).fields[index]!.element.scalarType)loss(location.path+'/scalarType',location.scalarType,'Native type does not establish ideal scalar family',true);});
 }
 ...
 result.mappings.unshift({origin:'authored',kind:'record',idealPath:selected.path+'/kind',nativePath:'/schema/0',outcome:result.residuals.length?'unknown':'exact'});
 if(hard||request.mode==='strict'&&result.residuals.length)result.status='blocked';else result.target=target!;
 const output=copyJson(result);if(!check(output))throw new UmfError('RECORD_PARQUET_RESULT',JSON.stringify(check.errors));return output as unknown as RecordParquetProjection;
}
export function recoverRecordFromParquet(input:RecordParquetProjection,nativeBytes:Uint8Array):Document {
 const receipt=copyJson(input) as unknown as RecordParquetProjection;
 if(!check(receipt)||receipt.status!=='projected'||!receipt.target)throw new UmfError('RECORD_PARQUET_RECEIPT','Expected complete projection receipt');
 const expected=projectRecordToParquet(receipt.author,receipt.request);
 ...
 if(canonical(copyJson(receipt))!==canonical(copyJson(expected)))throw new UmfError('RECORD_PARQUET_RECEIPT','Receipt differs from recomputed projection');
 const expectedBytes=exportParquetCapture(receipt.target);
 if(!(nativeBytes instanceof Uint8Array)||nativeBytes.length!==expectedBytes.length||nativeBytes.some((b,i)=>b!==expectedBytes[i]))throw new UmfError('RECORD_PARQUET_STALE','Native target changed');
