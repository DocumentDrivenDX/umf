...
export interface TableSpecFieldClassification {
 operation:'classify-tablespec-field';version:'1.0.0';status:'classified'|'blocked';source:Document;target?:Document;request:TableSpecFieldRequest;binding:typeof binding;diagnostics?:Diagnostic[];
...
export function classifyTableSpecField(input:Document,options:TableSpecFieldRequest):TableSpecFieldClassification & {diagnostics:Diagnostic[]} {
 ...
 const result:TableSpecFieldClassification={operation:'classify-tablespec-field',version:'1.0.0',status:'classified',source,request,binding,mapping:{origin:'classified',kind:'field',idealPath,nativePath,nativeFragment,basis:'checked-native-column-membership',outcome:'exact'},residuals:[],diagnostics:[]};
 ...
 return copied as unknown as TableSpecFieldClassification & {diagnostics:Diagnostic[]};
}
...
export function verifyTableSpecFieldClassification(input:TableSpecFieldClassification,current:Document):TableSpecFieldClassification {
 ...
 const expected:TableSpecFieldClassification=classifyTableSpecField(receipt.source,receipt.request);
 // Historical receipts predate diagnostics; retain their original shape while checking every original claim.
