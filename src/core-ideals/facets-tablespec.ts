...
export function classifyTableSpecFacets(input:Document,options:TableSpecFacetRequest):TableSpecFacetClassification {
 ...
 if(usable&&profile!=='gx-suite-spark'){
  ...
  if(hasPair){
   ...
   else if(profile==='declared-metadata'||profile==='ingest-cast'&&precision<=38||profile==='pyspark-schema'&&precision===10&&scale===0){
    result.mapping.facets.precision=precision;result.mapping.facets.scale=scale;observe('decimal','precision',profile==='declared-metadata'?'declared':'inferred','exact',profile==='declared-metadata'?'Explicit paired decimal declarations describe bounds only; native enforcement remains consumer-specific':profile==='pyspark-schema'?'Explicit decimal pair agrees with the native DecimalType(10,0) carrier; conversion may round':'Explicit Spark ingest decimal output domain; casting may round and ANSI mode changes overflow behavior');
