...
export interface FieldTableSpecProjection {
 operation:'project-field-tablespec';version:'1.0.0';status:'projected'|'blocked';source:Document;author:CoreKindDeclaration;request:FieldTableSpecRequest;target?:Document;binding:typeof binding;diagnostics?:Diagnostic[];
...
export function projectFieldToTableSpec(input:CoreKindDeclaration,options:FieldTableSpecRequest):FieldTableSpecProjection & {diagnostics:Diagnostic[]} {
 ...
 const result:FieldTableSpecProjection={operation:'project-field-tablespec',version:'1.0.0',status:'projected',source,author,request,binding,mapping:{origin:'authored',idealPath:path+'/kind',nativePath:'/columns/0',outcome:'exact'},residuals:[],diagnostics:[]};
 ...
 const output=copyJson(result);if(!check(output))throw new UmfError('FIELD_TABLESPEC_RESULT',JSON.stringify(check.errors));return output as unknown as FieldTableSpecProjection & {diagnostics:Diagnostic[]};
}
...
export function recoverFieldFromTableSpec(input:FieldTableSpecProjection,nativeText:string):Document {
 ...
 const expected:FieldTableSpecProjection=projectFieldToTableSpec(receipt.author,receipt.request);
 // Historical receipts predate diagnostics; this does not waive any original source/mapping check.
 if(!Object.hasOwn(receipt,'diagnostics'))delete expected.diagnostics;
