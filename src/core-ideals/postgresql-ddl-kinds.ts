...
export interface PostgresqlDdlKinds {
 operation:'classify-postgresql-ddl-record';version:'1.0.0';scope:'declared-only';status:'classified'|'blocked';source:Document;target?:Document;request:PostgresqlDdlKindRequest;binding:typeof binding;namespaceResolution:'explicit'|'create-schema-context'|'unresolved';
 mappings:{origin:'classified';kind:'field'|'record';idealPath:string;nativePath:string;nativeFragment:Json;basis:'checked-raw-column-declaration'|'checked-raw-table-declaration';outcome:'exact'|'unknown'}[];
...
export async function classifyPostgresqlDdlRecord(input:Document,options:PostgresqlDdlKindRequest,backend:PostgresqlBackend):Promise<PostgresqlDdlKinds>{
 ...
 const result:PostgresqlDdlKinds={operation:'classify-postgresql-ddl-record',version:'1.0.0',scope:'declared-only',status:'classified',source,request,binding,namespaceResolution:'unresolved',mappings:[],residuals:[],diagnostics:[{code:'POSTGRESQL_DECLARATION_ONLY',path:declaration.path,message:'Declared structure only; no catalog resolution, execution or later-statement replay',severity:'warning'}]};
 ...
 if(declaration.kind!=='create-table'||declaration.requiresCatalogExpansion)loss('POSTGRESQL_DDL_KIND_EXPANSION',declaration.path,declaration.nativeStatement,'Only explicit CREATE TABLE member lists without catalog expansion are covered');
 ...
 result.mappings.unshift({origin:'classified',kind:'record',idealPath:base+'/0/kind',nativePath:declaration.path,nativeFragment:copyJson(declaration.nativeStatement),basis:'checked-raw-table-declaration',outcome:result.residuals.length?'unknown':'exact'});
