...
resolution. RDF domain/range, an explicit OWL union-class domain with two
source classes, and LinkML slot/range/multivalued are likewise classified
native observations. The union fixture preserves its RDF list and blank-node
structure through native parsing and Chromium codec recovery; it does not
establish an authored heterogeneous source set. These fixtures do not count
