...
### Parquet composed recovery and acceptance entrypoints

Fresh-native composition now imports generated bytes without author metadata,
explicitly migrates the envelope and classifies the native value roles. For exact
projection claims, it checks every mapped shape, scalar family and selected
availability observation against the authored Field. Report-mode native shape
may differ from author intent; that difference is not erased. Both JSON/YAML
receipts recover the original bytes and retained author target independently.
The native matrix passes 264 fresh-native recoveries and 264 ideal recoveries.
Focused projection tests now pass 119 assertions, including mismatched native
shape with retained ideal recovery.

`bun scripts/core-ideals/cardinality-parquet-oracle.ts` is the combined native
verification entrypoint. It runs the native counterexamples, explicit-carrier
checks and projection/composition matrix, requires PyArrow 21.0.0, and verifies
child-proof and native-file fingerprints.
`UMF_CHROMIUM_PATH=/home/erik/.local/bin/chromium bun scripts/core-ideals/cardinality-parquet-browser.ts`
runs inspection, carrier, classification and composed projection checks in
Chromium 148, with fingerprint closure and no external requests.
The full shared-library refresh and separate conformance gates are still required
for qualified binding acceptance; Cardinality admission remains a separate gate.


### Parquet qualified binding acceptance
